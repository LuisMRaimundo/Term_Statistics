#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Revisão LR (GUIA_REVISAO_FASE1) — Análise 3 / Uniform_near.xlsx.

Preserva formatação amarela e listas do Excel original (openpyxl).
Saída: Uniform_near_revisto_LR.xlsx (mesma pasta).
"""
...
SRC = Path(
    ...
    r"\CLASSES TEXTURAIS\UNIFORM\Análise 3\Uniform_near.xlsx"
)
OUT = SRC.with_name("Uniform_near_revisto_LR.xlsx")
REVISOR = "LR"
...
FORMAS_EXCLUIR = {
    ...
    # truncamento OCR / match parcial de uniﬁcation / uniﬁed
    "uni": "ruido_ocr",
}
...
# «continuities with X texture» = continuidade histórica/cultural, não propriedade
RE_CONTINUITIES_HIST = re.compile(
    r"(?i)continuit(?:y|ies)\s+with\b"
)
...
def revisar(df: pd.DataFrame, cfg: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    # Falsos amigos / metatexto / domínio / leituras fora de classe
    for i, row in df.iterrows():
        ...
        if form in {"continuity", "continuities"} and RE_CONTINUITIES_HIST.search(ctx):
            if not re.search(r"(?i)textural\s+continuit", ctx):
                _excluir(
                    df, i, "fora_da_classe_uniforme",
                    "LR: continuity/continuities histórica/cultural, não propriedade uniforme",
                    stats, "continuities_hist",
                )
                continue
        ...
        # «uniformly X texture» — adverbial de modo/grau sobre atributo; manter nuclear
        # como adverbial se a extracção marcou atributiva por engano
        if form == "uniformly" and str(row.get("relacao_sintactica")) == "atributiva":
            df.at[i, "relacao_sintactica"] = "adverbial"
            _note(df, i, "LR: uniformly = adverbial (modo/grau) sobre textura")
            stats["uniformly_adverbial"] = stats.get("uniformly_adverbial", 0) + 1
    ...
    for i, row in df.iterrows():
        ...
        if _empty(row.get("polaridade")):
            if not _empty(row.get("polaridade_base")):
                ...
                # Classe UNIFORME: ausência de polaridade_base → estabilidade
        ...
        # Eixo fino para fixed / unchanging (invariância no tempo)
        if tipo in {"fixed", "fixe", "unchanging"} and str(df.at[i, "eixo"]) == "ambos":
            df.at[i, "eixo"] = "invariancia_diacronica"
            stats["eixo_fixed"] = stats.get("eixo_fixed", 0) + 1
...
def main() -> int:
    ...
    try:
        ...
        dest = OUT.with_name("Uniform_near_revisto_LR_FULL.xlsx")
        wb.save(dest)
        print(f"AVISO: destino bloqueado — gravei em {dest.name}", flush=True)

    # Também actualizar o ficheiro de trabalho Uniform_near.xlsx (pedido: return edited)
    try:
        wb.save(SRC)
        print(f"Actualizado in-place: {SRC}", flush=True)
    except PermissionError:
        print(
            f"AVISO: {SRC.name} bloqueado — revisão só em {dest.name}",
            flush=True,
        )
