...
| Campo | Nível | Notas |
|---|---|---|
| `source_matrix_row` | ocorrência | nº de linha Excel na matriz KWIC |
| `texture_occurrence_id` | ocorrência | `{doc_id}::ROW_{source_matrix_row}` |
| `match_id` | hit | `M001`… dentro da ocorrência |
| `hit_key` | hit | ocorrência + termo + forma + offsets |
| `idx_no` / `idx_termo` | hit | posição token no contexto da matriz |
| `off_no` / `off_termo` | hit | offset de carácter no contexto da matriz |
| `grupo_passagem_id` | passagem | janelas KWIC sobrepostas no mesmo `doc_id` |
| `candidato_duplicado` | flag | passagem/janela; revisão humana |

Fusão automática: só exclui (`janela_sobreposta`) hits com o **mesmo**
`canonical_term`+`matched_form` em janelas deslocadas. Termos diferentes na
mesma passagem mantêm-se (dois hits, contagens de ocorrência separadas por
linha da matriz).

Fase 2 — novos modos `--desduplicacao`: `ocorrencia`, `ocorrencia_termo`.
